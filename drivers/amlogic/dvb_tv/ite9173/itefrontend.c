...
static int ite9173_set_frontend(struct dvb_frontend *fe)
{
	struct aml_fe *afe = fe->demodulator_priv;
	struct aml_fe_dev *dev = afe->dtv_demod;
	...
	pr_dbg("c->frequency ==== %d bandwith==%d\n", c->frequency, bandwidth);
	if(freq>0&&freq!=-1) {
		mutex_lock(&ite_lock);
...
static int ite9173_get_frontend(struct dvb_frontend *fe)
...
static int ite9173_fe_enter_mode(struct aml_fe *fe, int mode)
{
	...
	pr_dbg("=========================demod init\r\n");
	amlogic_gpio_request(dev->reset_gpio, "ite9173_reset");
	amlogic_gpio_direction_output(dev->reset_gpio, dev->reset_value, "ite9173_reset");
	msleep(300);
	amlogic_gpio_direction_output(dev->reset_gpio, !dev->reset_value, "ite9173_reset");
	msleep(200);

	amlogic_gpio_request(dev->tuner_power_gpio, "ite9173_tunerpower");
	amlogic_gpio_direction_output(dev->tuner_power_gpio, 1, "ite9173_tunerpower");
	msleep(200);
