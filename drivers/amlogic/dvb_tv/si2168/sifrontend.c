...
static struct si2168_fe_config si2168_cfg;


int si2168_get_fe_config(struct si2168_fe_config *cfg){
	cfg->demod_addr=si2168_cfg.demod_addr;
	cfg->i2c_adapter=si2168_cfg.i2c_adapter;
...
static int si2168_fe_get_ops(struct aml_fe_dev *dev, int mode, void *ops)
{
	...
	si2168_cfg.demod_addr=dev->i2c_addr;
	si2168_cfg.i2c_adapter=dev->i2c_adap;
...
static int si2168_fe_resume(struct aml_fe_dev *dev)
{
	...
	amlogic_gpio_request(dev->reset_gpio,"si2168_reset");
	amlogic_gpio_direction_output(dev->reset_gpio, dev->reset_value, "si2168_reset");
	msleep(300);
	amlogic_gpio_direction_output(dev->reset_gpio, !dev->reset_value, "si2168_reset"); //enable tuner power
	msleep(200);
	int standard;
	standard=SILABS_DVB_T2;
	//Initialize LSI
