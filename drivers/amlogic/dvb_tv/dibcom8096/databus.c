...
static int data_bus_transfer_common(struct dibDataBusClient *client, struct dibDataBusAccess *msg)
{
	...
	if(0) // g_DebutI2cData)
	{
		if (host->type & DEBUG_TYPE)
		{
			...
			{
				dbgp("-W- ");
				dbgp("0x%02x] ", client->device_id);
				dbgp("[%ld]", msg->txlen);
				for(i=0;i<msg->txlen;i++)
				{
					dbgp(" 0x%02x ", msg->tx[i]);
				}

				if(msg->txlen == 3)
				{
					dbgp(" [%d: %d]", msg->tx[0], (msg->tx[1]<<8)|msg->tx[2]);
				}
				else
				{
					dbgp(" [%d: %d]", (msg->tx[0]<<8)|msg->tx[1], (msg->tx[2]<<8)|msg->tx[3]);
				}
				dbgp("\n");
				dbgp("\n");
			}
			else
			{
				if(msg->rxlen != 0)
				{
					...
					for(i=0;i<msg->txlen;i++)
					{
						dbgp("[w 0x%02x]", msg->tx[i]);
					}
					for(i=0;i<msg->rxlen;i++)
					{
						dbgp(" [r 0x%02x]", msg->rx[i]);
					}

					if(msg->txlen == 1)
					{
						dbgp(" [%d: ", msg->tx[0]);
						dbgp(" %d]", (msg->rx[0]<<8)|msg->rx[1]);
					}
					else
					{
						dbgp(" [%d: ", (msg->tx[0]<<8)|msg->tx[1]);
						dbgp(" %d]", (msg->rx[0]<<8)|msg->rx[1]);
					}
					dbgp("\n");
					dbgp("\n");
