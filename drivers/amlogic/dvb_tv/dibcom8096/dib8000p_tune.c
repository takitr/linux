...
void dib8000p_update_timf(struct dib8000p_state *state)
{
    ...
    v1 = dib8000p_read_word(state, 435);
    v2 = dib8000p_read_word(state, 436);
    timf = state->timf = (v1 << 16 )|(v2 & 0xffff);
        ...
        case CT_AGC_START:
            ...
            if (dib8000p_set_agc_config(state, (unsigned char)(BAND_OF_FREQUENCY(ch->RF_kHz))) != DIB_RETURN_SUCCESS) {
                *tune_state = CT_AGC_STOP;
                fe->status = FE_STATUS_TUNE_FAILED_DIB;
		break;
            }
    ...
    switch (*tune_state) {
        case CT_DEMOD_START: /* 30 */
            ...
            if (context->status == CHANNEL_STATUS_PARAMETERS_UNKNOWN) /* The channel struct is unknown, search it ! */
            {
                ...
                state->found_guard = GUARD_INTERVAL_AUTO_DIB;
        ...
        case CT_DEMOD_STEP_2:
            switch (state->autosearch_state) {
                ...
                case AS_SEARCHING_GUARD:
                    ...
                    fe->status = FE_STATUS_FFT_SUCCESS_DIB; /* signal to the upper layer, that there was a channel found and the parameters can be read */
                    *tune_state = CT_DEMOD_STEP_3;
                    break;
                default:
        ...
        case CT_DEMOD_STEP_4: /* (34) */
            ...
            if (fe->status > FE_STATUS_FFT_SUCCESS_DIB)
        ...
        case CT_DEMOD_STEP_6: /* (36)  if there is an input (diversity) */
            if(fe->input != NULL && state->output_mode != OUTPUT_MODE_DIVERSITY) {
                ...
                if ( fe->input->status <= FE_STATUS_STD_SUCCESS_DIB) /* Something is locked on the input fe */
                    *tune_state = CT_DEMOD_STEP_8; /* go for mpeg */
                else if (fe->input->status >= FE_STATUS_TUNE_TIME_TOO_SHORT_DIB ) { /* fe in input failled also, break the current one */
        ...
        case CT_DEMOD_STEP_8: /* 38 */
            ...
            if (ch->u.isdbt.sb_mode && ch->u.isdbt.sb_subchannel == -1 && !state->differential_constellation) {
                ...
                *tune_state = CT_DEMOD_STEP_9;
                fe->status = FE_STATUS_LOCKED_DIB;
            }
            break;
        ...
        case CT_DEMOD_STEP_10: /* 40 */
            if (locks&(1<<(7-state->longest_intlv_layer))) { /* mpeg lock : check the longest one */
                ...
                if (ch->u.isdbt.sb_mode && ch->u.isdbt.sb_subchannel == -1 && !state->differential_constellation)
                    ...
                    fe->status = FE_STATUS_DEMOD_SUCCESS_DIB;
                else
                    fe->status = FE_STATUS_DATA_LOCKED_DIB;
...
void dib8000p_autosearch_start(struct dibFrontend *fe, struct dibChannel *ch)
{
    ...
    if (state->autosearch_state == AS_SEARCHING_FFT) {
        ...
        schan.u.isdbt.guard                  = GUARD_INTERVAL_1_8_DIB;
