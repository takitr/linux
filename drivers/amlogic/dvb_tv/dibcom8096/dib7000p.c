...
static int dib7000p_get_signal_status(struct dibFrontend *fe, struct dibDVBSignalStatus *status)
{
	...
	int16_t equal_noise_exp = 0;
	int16_t equal_signal_exp = 0;

	v = dib7000p_read_word(st,509);
	...
	//
	// Used for calc signel quality
	//
	val = dib7000p_read_word(st, 479);
	status->equal_noise_mant = (val >> 4) & 0xFF;
	equal_noise_exp = ((val & 0xF) << 2);

	val = dib7000p_read_word(st, 480);
	equal_noise_exp += ((val >> 14) & 0x3);
	if ((equal_noise_exp & 0x20) != 0)
		equal_noise_exp -= 0x40;

	status->equal_signal_mant = (val >> 6) & 0xFF;
	equal_signal_exp  = (val & 0x3F);
    if ((equal_signal_exp & 0x20) != 0)
		equal_signal_exp -= 0x40;

	status->equal_noise_exp = equal_noise_exp;
	status->equal_signal_exp = equal_signal_exp;

#if 0
    uint32_t equal_noise_dB = 0;
	uint32_t equal_signal_dB = 0;
    uint32_t CoN = 0;

    equal_signal_dB = 10 * log10((equal_signal_mant+1e-10) * pow(2.0,equal_signal_exp));
    equal_noise_dB = 10 * log10((equal_noise_mant+1e-10) * pow(2.0,equal_noise_exp));

    CoN = equal_signal_dB - equal_noise_dB;
	...
	else if(CoN < 30 && CoN > 15)
	{
		status->V_agc_1 = (((CoN - 15) * 100) / (30 - 15));
	}
#endif

	//
	// Used for calc signel strength
	//
