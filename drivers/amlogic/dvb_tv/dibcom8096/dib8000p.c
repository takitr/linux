...
static int dib8000p_get_signal_status(struct dibFrontend *fe, struct dibDVBSignalStatus *status)
{
    ...
    {
    	  v1 = dib8000p_read_word(st, 562);
	  v2 = dib8000p_read_word(st, 563);
	  status->bit_error_rate = (v1 << 16 )|(v2 & 0xffff);
...
int32_t dib8000p_get_adc_power(struct dibFrontend *fe, uint8_t mode)
{
    ...
    v1 = dib8000p_read_word(state, 384);
    v2 = dib8000p_read_word(state, 385);
    val = (v1 << 16 )|(v2 & 0xffff);
...
static int dib8000p_get_channel(struct dibFrontend *fe, struct dibChannel *cd)
{
    ...
    switch (v) {
        case 0: cd->u.isdbt.guard = GUARD_INTERVAL_1_32_DIB; break;
        case 1: cd->u.isdbt.guard = GUARD_INTERVAL_1_16_DIB; break;
        case 2: cd->u.isdbt.guard = GUARD_INTERVAL_1_8_DIB ; break;
        default:
        case 3: cd->u.isdbt.guard = GUARD_INTERVAL_1_4_DIB ; break;
    }
...
extern uint32_t dib8000p_get_internal_freq(struct dib8000p_state *state)
{
   // return dib8000p_read32(state, 23) /1000;
    ...
    //val = dib8000p_read32(state, 384);
    v1 = dib8000p_read_word(state, 23);
    v2 = dib8000p_read_word(state, 24);
    val = (v1 << 16 )|(v2 & 0xffff);
