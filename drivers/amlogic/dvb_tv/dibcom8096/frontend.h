...
#define SIP_LAYER_RELEASE         0x2
    ...
    enum frontend_tune_state_dib tune_state;
    uint32_t current_rf;
    uint32_t current_bw;
...
#define FE_STATUS_TUNE_FAILED_DIB          0 // No channel was found in the given time
#define FE_STATUS_TUNE_TIMED_OUT_DIB      -1 // Tune process is not finished in the given time : continue the process later with some more time
#define FE_STATUS_TUNE_TIME_TOO_SHORT_DIB -2 // The given time was to short to make the tune process says something about the channel
#define FE_STATUS_TUNE_PENDING_DIB        -3 // Tune process is pending
#define FE_STATUS_STD_SUCCESS_DIB         -4 // A channel has been found, we know the standard, we can continue
#define FE_STATUS_FFT_SUCCESS_DIB         -5 // A channel has been found, we know the main params (Ie: FFT, Guard...) to continue
#define FE_STATUS_DEMOD_SUCCESS_DIB       -6 // A channel has been found, we are able to receive all channel params (TPS, TMCC, FIC..)
#define FE_STATUS_LOCKED_DIB              -7 // The found channel is now completly and successfully locked, but may not be stable
#define FE_STATUS_DATA_LOCKED_DIB         -8 // The found channel is now completly and successfully locked, and datas may output correctly
...
extern int component_ready(uint32_t t, enum frontend_tune_state_dib st0, enum frontend_tune_state_dib st1);
