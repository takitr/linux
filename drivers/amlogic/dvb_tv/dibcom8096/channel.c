...
static void init_dvbt_channel(struct dibChannel *ch)
{
    ...
    ch->u.dvbt.guard              = GUARD_INTERVAL_AUTO_DIB;
    ch->u.dvbt.spectrum_inversion = -1;
    ch->u.dvbt.constellation      = QAM_AUTO_DIB;
...
static void init_dvbsh_channel(struct dibChannel *ch)
{
    ...
    ch->u.dvbsh.dvb_common.guard              = GUARD_INTERVAL_AUTO_DIB;
    ch->u.dvbsh.dvb_common.spectrum_inversion = -1;
    ch->u.dvbsh.dvb_common.constellation      = QAM_AUTO_DIB;
...
static void init_isdbt_channel(struct dibChannel *ch)
{
    ...
    ch->u.isdbt.guard               = GUARD_INTERVAL_AUTO_DIB;
    ...
    for (i = 0; i < 3; i++) {
        ch->u.isdbt.layer[i].constellation = QAM_AUTO_DIB;
...
void init_atsc_channel(struct dibChannel *ch) {
    ch->bandwidth_kHz             = 6000;
	ch->u.atsc.modulation         = QAM_AUTO_DIB;
