...
{
    int ret,i,k = 0,stat = 0, num_failed = 0, succeeded = 0;
...
void frontend_tune_diversity_async(struct dibFrontend fe[], int count, struct dibChannel *ch)
{
    ...
    do {
		...
		if ((frontend_get_status(&fe[i]) == FE_STATUS_DEMOD_SUCCESS_DIB) || (frontend_get_status(&fe[i]) == FE_STATUS_FFT_SUCCESS_DIB)) {
        ...
        if (frontend_get_status(&fe[0]) == FE_STATUS_TUNE_FAILED_DIB ||
                frontend_get_status(&fe[0]) == FE_STATUS_LOCKED_DIB ||
                frontend_get_status(&fe[0]) == FE_STATUS_DATA_LOCKED_DIB) {
            active = 0;
        ...
        while ((active == 1) && (systime() < callback_time)) { DibUSleep(100); /* utilize CPU */}
