...
#endif
    fe->status = FE_STATUS_TUNE_PENDING_DIB;
...
int component_ready(uint32_t t, enum frontend_tune_state_dib st0, enum frontend_tune_state_dib st1)
{
...
void frontend_tune(struct dibFrontend *fe, struct dibChannel *ch)
{
...
#ifdef WAVE_ADAPT
    enum frontend_tune_state_dib *tune_state = &fe->tune_state;
#endif
    ...
    if(fe->status == FE_STATUS_LOCKED_DIB)
        fe_info->tune_time_locked = systime() - fe_info->tune_start_time;

    if(fe->status == FE_STATUS_DATA_LOCKED_DIB && (fe->tune_state == CT_DEMOD_STOP || fe->tune_state == CT_CHAN_DEC_STOP))
    ...
    if((fe->tune_state == CT_DEMOD_STOP || fe->tune_state == CT_CHAN_DEC_STEP_LOCKED) && fe->status == FE_STATUS_TUNE_FAILED_DIB) {
    ...
    if((fe->tune_state == CT_DEMOD_STOP || fe->tune_state == CT_CHAN_DEC_STOP) && (fe->status == FE_STATUS_DATA_LOCKED_DIB || fe->status == FE_STATUS_LOCKED_DIB)) {
        alpha = 8;
    ...
    for (segment_index = 1; segment_index <= total_segment_number; segment_index++) {
        ...
        do {
            ...
            for (i = 0; i < num; i++) {
                ...
                if (fe[i]->status_has_changed && (frontend_get_status(fe[i]) == FE_STATUS_DEMOD_SUCCESS_DIB || frontend_get_status(fe[i]) == FE_STATUS_FFT_SUCCESS_DIB)) {
                ...
                if (frontend_get_status(fe[i]) == FE_STATUS_LOCKED_DIB) {
                    subch_id = ch[channel_index].u.isdbt.sb_subchannel;
                    channel_index++;
                    success = 1;
                    break;
                } else if(frontend_get_status(fe[i]) == FE_STATUS_TUNE_FAILED_DIB) {
                    fe_fail_count++;
