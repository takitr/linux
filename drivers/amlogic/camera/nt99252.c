...
static struct v4l2_fract nt99252_frmintervals_active = {
    .numerator = 1,
    .denominator = 15,
};
...
static struct v4l2_frmivalenum nt99252_frmivalenum[]={
	 {
		 .index 		= 0,
...
static void NT99252_init_regs(struct nt99252_device *dev)
{
	struct i2c_client *client = v4l2_get_subdevdata(&dev->sd);
    int i=0;
...
static int set_flip(struct nt99252_device *dev)
{
	struct i2c_client *client = v4l2_get_subdevdata(&dev->sd);
	unsigned char temp;
	...
	{		
	  if((height<=600)&&(width<=800))
	  {
			...
			nt99252_v_active=598;	
                        nt99252_frmintervals_active.denominator = 15;
                        nt99252_frmintervals_active.numerator	= 1;
			#endif			
		}
		else
		{
			...
			#endif
			msleep(10);// 10
                        nt99252_frmintervals_active.denominator = 5;
                        nt99252_frmintervals_active.numerator	= 1;
        ...
        struct v4l2_frmivalenum *fival)
{
    ...
    for(k =0; k< ARRAY_SIZE(nt99252_frmivalenum); k++)
    {
        if( (fival->index==nt99252_frmivalenum[k].index)&&
                (fival->pixel_format ==nt99252_frmivalenum[k].pixel_format )&&
                (fival->width==nt99252_frmivalenum[k].width)&&
                (fival->height==nt99252_frmivalenum[k].height)){
            memcpy( fival, &nt99252_frmivalenum[k], sizeof(struct v4l2_frmivalenum));
            return 0;
        }
    }
        ...
        struct v4l2_streamparm *parms)
{
    ...
    cp->timeperframe = nt99252_frmintervals_active;
...
static int vidioc_streamon(struct file *file, void *priv, enum v4l2_buf_type i)
{
	...
	para.frame_rate = nt99252_frmintervals_active.denominator;//175;
			...
			const struct i2c_device_id *id)
{
	...
	if (plat_dat) {
             ...
             printk("camera nt99252: have no platform data\n");
             kfree(t);
             return -1;     
	}
